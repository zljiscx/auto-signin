...
class BuddyExecutor(BaseExecutor):
    mode = 'buddy'
    label = 'Buddy令牌签到'
    description = 'WorkBuddy/Buddy加油站令牌式签到：粘贴token_store或读取本机登录态，自动续期保持永久登录，按本项目时间表执行'
    ...
    def run(self):
        ...
        return SignResult(True, msg, None, detail)
    ...
    def _fetch_status(self, token, uid, domain):
        """查询签到状态；成功返回 data 字典，失败返回 {}。"""
        st, body = self._call(ENDPOINT_STATUS, token, uid, domain, method="GET")
        if st == 200:
            try:
